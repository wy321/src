...
def callback_cly(data_cly):
	...
	if len(data_cly.cylinders) != 0:
		for i in range(len(data_cly.cylinders)):
			cly_dis = np.array([data_cly.cylinders[i].label, data_cly.cylinders[i].Xrobot, data_cly.cylinders[i].Zrobot])
			s_xx = data_cly.cylinders[i].covariance[0]
			s_xy = data_cly.cylinders[i].covariance[1]
			s_yy = data_cly.cylinders[i].covariance[3]
			
			line = ("LANDMARK_MEAS2D " + str(id_landmark) + " " + str(cly_dis[1]) + " " + str(cly_dis[2]) + " " 
			+ str(s_xx) + " " + str(s_xy) + " " + str(s_yy) + "\n")
			
			measurement_file.write(line)
			#print(line)
			
			cly_abs = cly_rel2abs(robot_abs,cly_dis)
			line2 = ("POINT2D " + str(id_landmark) + " " + str(cly_abs[1]) + " " + str(cly_abs[2]) + "\n")
			print line2
			print line
